/**
 * Main entry for the scraper process: runs BrettZone and TrueFinals scrapers either once or on a dynamic cron schedule.
 *
 * Usage:
 ...
 * - `ts-node runScrapers.ts --once` or `... run-now`: run both scrapers once in sequence (BrettZone then TrueFinals) then exit(0).
 *
 * Preconditions:
