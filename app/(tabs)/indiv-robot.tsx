            ...
            <ScrollView>
                ...
                {visibleFights.length === 0 ? (
                    ...
                    visibleFights.map((f, idx) =>
                        <IndivFightCard key={idx} props={{ title: `vs ${f?.opponent_name}`, photoUrl: photoUrl!, fstText: isUpcoming ? `Live at: ${(f?.fight_time ? f?.fight_time.toLocaleString() : 'TBD')}` : `Fought at: ${f?.fight_time}`, innerBox: isUpcoming ? (f?.cage ? `Cage: ${f?.cage}` : 'Cage: TBD') : f?.is_win === "win" ? `Win` : `Loss` }} />)
                )}
            </ScrollView>
            <TouchableOpacity
